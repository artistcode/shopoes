...
jQuery(function ($) {
	...
	$('.swiper').each(function () {
		...
		if ($content.has($this).length) {
			c = 'test.html' + window['Journal']['columnsCount'];
	...
	if ($html.hasClass('route-product-product')) {
		...
		$(document).delegate('.additional-image', 'click', function () {
			...
			let img =  this.children[0].cloneNode();
			let src = img.getAttribute('src');
			 img.src = src.replace(/80x80/, '550x550');

			let srcset =img.getAttribute('srcset');
			console.log(srcset);
			let a = srcset.replace(/160x160/,'1000x1000');

			img.srcset = a.replace('80x80','550x550');
		$('#large_picture').html(img);
